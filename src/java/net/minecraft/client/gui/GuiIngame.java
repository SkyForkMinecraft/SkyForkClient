...
public class GuiIngame extends Gui
{
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer && !Access.getInstance().getModuleManager().getHandle(CustomHotbar.class).isEnabled()) {
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        ...
        if (mc.getRenderViewEntity() instanceof EntityPlayer && Access.getInstance().getModuleManager().getHandle(CustomHotbar.class).isEnabled()) {
            ...
            GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
            RenderHelper.enableGUIStandardItemLighting();
            CustomHotbar.renderEffect(sr);
            for (int j = 0; j < 9; ++j) {
                int k = sr.getScaledWidth() / 2 - 90 + j * 20 + 2;
                int l = (int) (sr.getScaledHeight() - (19 + 15 * GuiChat.openingAnimation.getOutput().floatValue()));
                this.renderHotbarItem(j, k, l - 4, partialTicks, entityplayer);
            }
        }
    }
    ...
    public void renderExpBar(ScaledResolution scaledRes, int x) {
        if (Access.getInstance().getModuleManager().getHandle(CustomHotbar.class).isEnabled()) return;
    ...
    public void renderSelectedItem(ScaledResolution scaledRes) {
        ...
        if (this.remainingHighlightTicks > 0 && this.highlightingItemStack != null) {
            ...
            float i = (float) (scaledRes.getScaledWidth() - getFontRenderer().getStringWidth(s)) / 2;
            ...
            if (k > 0) {
                getFontRenderer().drawStringWithShadow(s, i, j - 15 * GuiChat.openingAnimation.getOutput().floatValue(), ColorUtil.applyOpacity(Color.WHITE.getRGB(), k / 255f));
            }
        }
