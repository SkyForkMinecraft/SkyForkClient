...
public class MusicOverlayRenderer {
    ...
    public void renderOverlay(float x, float y) {
        ...
        if (MusicManager.INSTANCE.getCurrentTrack() != null && MusicManager.INSTANCE.getMediaPlayer() != null) {
            FontManager.M18.drawString(MusicManager.INSTANCE.getCurrentTrack().name + " - " + MusicManager.INSTANCE.getCurrentTrack().artists, x + 32, y + 5, Color.WHITE.getRGB());
            FontManager.M18.drawString(formatSeconds((int) readSecs) + "/" + formatSeconds((int) totalSecs), x + 32, y + 15, 0xffffffff);
        ...
        if (MusicManager.INSTANCE.visualize) {
            if (MusicManager.INSTANCE.magnitudes != null) {
                ...
                for (int i = 0; i < 100; i += 1) {
                    ...
                    RenderUtil.drawRect(i * width, sr.getScaledHeight(), width, -VisWidth[i], new Color(0x1FFFFFFF, true).getRGB());
