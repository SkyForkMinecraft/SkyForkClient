...
import com.skyfork.api.yapeteam.ClickUI.component.impl.slots.TrackSlotComponent;
import com.skyfork.api.yapeteam.cloudmusic.MusicManager;
import com.skyfork.api.yapeteam.cloudmusic.api.CloudMusicAPI;
import com.skyfork.api.yapeteam.cloudmusic.impl.Track;
import com.skyfork.api.yapeteam.cloudmusic.ui.QRLoginScreen;
...
public class MusicPlayerComponent implements com.skyfork.api.yapeteam.ClickUI.component.Component {
    private final FontDrawer icon = FontManager.icon30;
    ...
    public void draw(float x, float y, float mouseX, float mouseY) {
        ...
        RoundedUtil.drawGradientHorizontal(x, y + (ClickUIScreen.height - 50), ClickUIScreen.width - ClickUIScreen.leftWidth - 10, 45, 5, setAlpha(HUD.getClientColors().getFirst()), setAlpha(HUD.getClientColors().getSecond()));
        ...
        if (!showList) {
            if (MusicManager.INSTANCE.magnitudes != null) {
                for (int i = 0; i < 100; i++) {
                    ...
                    RenderUtil.drawRect(x, y + 20 + 5 * i, VisWidth[i], 3, 0x80000000);
                    if (MusicManager.INSTANCE.getMediaPlayer().getStatus() == MediaPlayer.Status.PLAYING)
                        VisWidth[i] += (float) ((MusicManager.INSTANCE.magnitudes[i] - VisWidth[i]) / (1.7 * (Minecraft.getDebugFPS() / 60f)));
            ...
            GradientUtil.applyGradientHorizontal(x + ((ClickUIScreen.width - ClickUIScreen.leftWidth - FontManager.M18.getStringWidth(songName + " | " + songArtist)) / 2f), y + (ClickUIScreen.height - 100) / 2f - 50 + 100 + 5, FontManager.M18.getStringWidth(songName + " | " + songArtist), FontManager.M18.getHeight(), ClickUIScreen.globalAlpha / 255f, HUD.getClientColors().getFirst(), HUD.getClientColors().getSecond(), () ->
                    FontManager.M18.drawString(songName + " | " + songArtist, x + ((ClickUIScreen.width - ClickUIScreen.leftWidth - FontManager.M18.getStringWidth(songName + " | " + songArtist)) / 2f), y + (ClickUIScreen.height - 100) / 2f - 50 + 100 + 5, globalBlack.getBlue())
            );

            if (MusicManager.INSTANCE.currentTrack != null) {
                if (MusicManager.INSTANCE.getArt(MusicManager.INSTANCE.currentTrack.id) != null) {
                    GL11.glPushMatrix();
                    RenderUtil.drawImage(MusicManager.INSTANCE.getArt(MusicManager.INSTANCE.currentTrack.id), (int) (x + ((ClickUIScreen.width - ClickUIScreen.leftWidth - 100) / 2f)), (int) (y + (ClickUIScreen.height - 100) / 2f - 50), 100, 100, ClickUIScreen.globalAlpha / 255f);
                    GL11.glPopMatrix();
                }
            }
        }
        ...
        if (showList) {
            ...
            RoundedUtil.drawRound(x + ClickUIScreen.width - ClickUIScreen.leftWidth - 37, y + 20 + 3 + 1.5f, 26, 14, 2, isHoveredIn(x + ClickUIScreen.width - ClickUIScreen.leftWidth - 37, y + 20 + 3 + 1.5f, 26, 14, mouseX, mouseY) ? setAlpha(new Color(206, 206, 206)).getRGB() : setAlpha(new Color(206, 206, 206)).brighter().getRGB());
            FontManager.M14.drawString("Search", x + ClickUIScreen.width - ClickUIScreen.leftWidth - 37 + 2, y + 20 + 3 + 1.5f + 3, globalBlack.getRGB());
            if (textField.textString.isEmpty()) {
                ...
                FontManager.M18.drawString(text, x + 10, y + 20 + 3 + 5, setAlpha(Color.GRAY).getRGB());
            }
        }
        //processBar
        float progress = 0;
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        if (mouseButton == 0) {
            ...
            if (isHoveredIn(x + 5 + 2 + icon.getStringWidth("P"), y + 5 + (ClickUIScreen.height - 50), icon.getStringWidth("R"), icon.getHeight(), mouseX, mouseY)) { // QRCode
                mc.displayGuiScreen(new QRLoginScreen(Access.getInstance().getClickGui()));
            }
    ...
    public void mouseReleased(float mouseX, float mouseY, int state) {
        if (showList) for (com.skyfork.api.yapeteam.ClickUI.component.Component component : SubComponent) {
