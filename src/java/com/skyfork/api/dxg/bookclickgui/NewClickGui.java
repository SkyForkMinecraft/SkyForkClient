package com.skyfork.api.dxg.bookclickgui;
...
public class NewClickGui extends GuiScreen
{
    ...
    @EventTarget
    public void onShader(final ShaderEvent e) {
        if (e.getType() == ShaderType.Shadow) {
            RoundedUtil.drawRound((float)this.x, (float)this.y, 530.0f, 330.0f, 4.0f, new Color(0, 0, 0));
        }
    }
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        RoundedUtil.drawRound((float)this.x, (float)this.y, 530.0f, 330.0f, 4.0f, this.getColor(24, 28, 31));
        RoundedUtil.drawRound((float)this.x, (float)this.y, 530.0f, 330.0f, 4.0f, this.getColor(24, 28, 31));
        RoundedUtil.drawRound((float)this.x, (float)(this.y + 300), 120.0f, 30.0f, 12.0f, this.getColor(34, 38, 41));
        Mask.defineMask();
        ...
        if (this.page.isCategoryPage) {
            RoundedUtil.drawRound((float)(this.x + 4), (float)(this.y + 32), 112.0f, 20.0f, 4.0f, this.getColor(48, 51, 54));
            if (!this.search) {
                RoundedUtil.drawRound(this.x + 112 - 16, this.y + 34, 16, 16, 4.0f, this.getColor(24, 28, 31).getRGB());
                FontManager.arial22.drawString("^F", (float)(this.x + 112 - 15), this.y + 38.0f, this.getColor(new Color(HUD.mainColor.getColor())).getRGB());
                FontManager.arial18.drawString("Search Module..", (float)(this.x + 8), (float)(this.y + 39), MouseUtil.isHovering((float)(this.x + 4), (float)(this.y + 32), 112.0f, 20.0f, mouseX, mouseY) ? this.getColor(255, 255, 255, 201).getRGB() : this.getColor(255, 255, 255, 180).getRGB());
            }
            else {
                try {
                    if (StringUtils.isEmpty((CharSequence)(this.searching).getValue())) {
                        FontManager.arial18.drawString("Search Module..", (float)(this.x + 8), (float)(this.y + 39), MouseUtil.isHovering((float)(this.x + 4), (float)(this.y + 32), 112.0f, 20.0f, mouseX, mouseY) ? this.getColor(255, 255, 255, 60).getRGB() : this.getColor(255, 255, 255, 50).getRGB());
                    }
                    FontManager.arial18.drawString(this.searching.getValue(), (float)(this.x + 8), (float)(this.y + 37), MouseUtil.isHovering((float)(this.x + 4), (float)(this.y + 32), 112.0f, 20.0f, mouseX, mouseY) ? this.getColor(255, 255, 255, 201).getRGB() : this.getColor(255, 255, 255, 180).getRGB());
                }
                catch (Exception ex) {}
            }
            this.searchRipple.draw(() -> RoundedUtil.drawRound(this.x + 4, this.y + 32, 112, 20, 4.0f, this.getColor(48, 51, 54).getRGB()));
        }
        ...
        for (final Page page : Page.values()) {
            Label_1477: {
                ...
                if (RenderUtil.isHovering((float)(this.x + 4), (float)cy, 112.0f, 20.0f, mouseX, mouseY)) {
                    ...
                    RoundedUtil.drawRound(this.x + 4, cy, 112, 20, 4.0f, this.getColor(45, 50, 58).getRGB());
                }
                else {
                ...
                if (page == this.page) {
                    ...
                    RoundedUtil.drawRound(this.x + 4, cy, 112, 20, 4.0f, this.getColor(RenderUtil.reAlpha(new Color(HUD.mainColor.getColor()).darker().darker(), 201)).getRGB());
                }
                final int finalCy = cy;
                page.ripple.draw(() -> RoundedUtil.drawRound(this.x + 4, finalCy, 112, 20, 4.0f, this.getColor(41, 39, 51).getRGB()));
    ...
    private float drawOptions(float my, final Value<?> option, final int mouseX, final int mouseYi) {
        ...
        if (option instanceof ModeValue) {
            ...
            RoundedUtil.drawRound(x + 125.0f, my + 10.0f, 392.0f, (float)(16 + ((opt3.expanded && opt3.getModes().length > 1) ? (16 * (opt3.getModes().length - 1)) : 0)), 2.0f, this.getColor(0, 0, 0, 40));
            if (opt3.expanded) {
                float comboY = my + 30.0f;
                for (final Enum s : opt3.getModes()) {
