...
package com.skyfork.api.florianmichael.viamcp;

import com.skyfork.api.florianmichael.vialoadingbase.ViaLoadingBase;
import com.skyfork.api.florianmichael.viamcp.gui.AsyncVersionSlider;
    ...
    public ViaMCP() {
        ViaLoadingBase.ViaLoadingBaseBuilder.create().runDirectory(new File("ViaMCP")).nativeVersion(NATIVE_VERSION).onProtocolReload(protocolVersion -> {
            if (asyncVersionSlider != null) {
                asyncVersionSlider.setVersion(protocolVersion.getVersion());
            }
        }).build();
