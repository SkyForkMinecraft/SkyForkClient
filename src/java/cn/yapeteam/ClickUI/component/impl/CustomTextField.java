...
public class CustomTextField {
    ...
    public void draw(float x, float y, int alpha) {
        ...
        RoundedUtil.drawRound(this.x, this.y + 3f, this.x + width, this.y + height, 1, new Color(241, 241, 241, alpha).getRGB());//115,15
        FontManager.M18.drawString(this.textString, this.x + 1.5f - this.offset, this.y + (height - FontManager.M18.getHeight()) / 2f + 2, setAlpha(Color.GRAY, alpha).getRGB());
        if (this.isFocused) {
            float width = FontManager.M18.getStringWidth(this.textString.substring(0, selectedChar)) + 4;
            float posX = this.x + width - this.offset;
            RenderUtil.drawRect(posX - 2.5f, this.y + 5.5f, posX - 2f, this.y + height - 2.5f, ColorUtil.reAlpha(setAlpha(Color.GRAY, alpha).getRGB(), ticks / 500 % 2 == 0 ? 1f : 0));
        }
        this.tick();
    }
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseID) {
        boolean hovering = MouseUtil.isHovering(this.x, this.y + 3f, this.x + width, this.y + height, mouseX, mouseY);
        ...
        if (this.isFocused) {
            ...
            switch (key) {
                ...
                case Keyboard.KEY_INSERT:
                    ...
                    this.selectedChar = this.textString.length();
                    width = FontManager.M18.getStringWidth(this.textString.substring(0, this.selectedChar))
                            + 2;
                ...
                case Keyboard.KEY_BACK:
                    try {
                        ...
                        if (!this.textString.isEmpty()) {
                            ...
                            --this.selectedChar;
                            if (FontManager.M18.getStringWidth(oldString) + 2 > this.width - 4 && this.offset > 0.0F) {
                                newTextWidth = FontManager.M18.getStringWidth(this.textString) + 2;
                                oldTextWidth = FontManager.M18.getStringWidth(oldString) + 2;
                ...
                case Keyboard.KEY_RIGHT:
                    ...
                    width = FontManager.M18.getStringWidth(this.textString.substring(0, this.selectedChar)) + 2;
                    barOffset = width - this.offset;
    ...
    public void charTyped(char c) {
        if (this.isFocused && ChatAllowedCharacters.isAllowedCharacter(c)) {
            ...
            if (this.selectedChar > this.textString.length()) {
                ...
                ++this.selectedChar;
                float width = FontManager.M18.getStringWidth(this.textString.substring(0, this.selectedChar)) + 2;
