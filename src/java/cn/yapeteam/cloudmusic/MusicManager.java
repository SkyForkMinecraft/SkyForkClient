package cn.yapeteam.cloudmusic;
...
public class MusicManager {
    ...
    public MusicManager() {
        ...
        musicFolder = new File(mc.mcDataDir, "SkyForkMusic/musicCache");
        artPicFolder = new File(mc.mcDataDir, "SkyForkMusic/artCache");
        File cookie = new File(mc.mcDataDir, "SkyForkMusic/cookies.txt");

        if (!musicFolder.exists())
            musicFolder.mkdirs(); // 文件夹不存在时创建

        circleImage = new File(Minecraft.getMinecraft().mcDataDir.toString() + File.separator + Access.CLIENT_NAME + File.separator + "circleImage");
        ...
        if (!mp3File.exists() && !flacFile.exists()) {
            ...
            loadingThread = new Thread(() -> {
                try {
                    ...
                    ChatUtil.send("缓存音乐时发生错误, 可能是因为该歌曲已被下架或需要VIP!");
                    if (mp3File.exists())
                        mp3File.delete();
