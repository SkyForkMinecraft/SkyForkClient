...
{
    return (x - resolution.xy / 2.0) / resolution.y * 1000.0 + 500.0;
}
...
vec4  fC ( in vec2 fragCoord ) // Change the void mainImage of your shader to this.
{
    ...
    if (!(ear(c, p) || mane(c, p) || mane2(c, p) || eye1(c, p, vec2(off*1.6,off*.5)) || eye2(c, p, vec2(off*.4,off*.5)) || face(c, p) || neck(c, p))) {
        vec3 pinkColor = vec3(1.0, 0.61, 0.776);
        vec3 blueColor = vec3(0.36, 0.8, 1.0);
        vec3 whiteColor = vec3(1.0, 1.0, 1.0);

        if (fragCoord.y < resolution.y / 5.0)
        c = blueColor;
        else if (fragCoord.y < 2.0 * resolution.y / 5.0)
        c = pinkColor;
        else if (fragCoord.y < 3.0 * resolution.y / 5.0)
        c = whiteColor;
        else if (fragCoord.y < 4.0 * resolution.y / 5.0)
        c = pinkColor;
        else
        c = blueColor;
    }
